...
// If the image is hosted on cloudinary
...
module.exports = async (
  ...
  const generateImagesAndUpdateNode = async function(node, resolve) {
    ...
    if (node.url.indexOf(`cloudinary.com`) === -1) {
      return resolve()
    }
    ...
    const cacheKey = `remark-images-cloudinary-${fileName}-${optionsHash}`
        ...
        node =>
          new Promise(async (resolve, reject) => {
            ...
            for (let thisImg of imageRefs) {
              ...
              if (formattedImgTag.url.indexOf(`cloudinary.com`) !== -1) {
